...
## 4. 오버로딩
  ...
  + 오버로딩의 장점
    - 같은 기능을 하는 메서드들이지만, 서로 다른 이름을 강제로 가져야 하기 때문에 그 불편함을 제거해준다
  + 가변인자(varargs)와 오버로딩
    - 메서드의 매개변수의 개수가 고정적이였으나 JDK1.5부터 동적으로 지정해 줄 수 있게 되었으며, 이 기능을 '가변인자'라고 함
    - 가변인자는 항상 제일 마지막에 선언되어야 한다
      ```java
